<?php
    if ( $_SESSION['user']['role'] == "admin" or $_SESSION['user']['role'] == "observe" ) {
        $datGoodData = sqlQuery("SELECT id, screenname FROM users WHERE role = 'org';");
        $currentUserQuery = sqlQuery("SELECT screenname FROM users WHERE id = " . $_SESSION['viewing_user_id'] . ";")[0];
        echo '
